...
const ExperienceLevel = (props: ExperienceLevelProps) => {
  const stepSix = props?.onChange("Six");
  const [disabled, setDisabled] = useState(true);

  useEffect(() => {
    if (
      props?.data?.stepSix?.experience && props?.data?.stepSix?.gym
    ) {
      setDisabled(false);
    } else {
      setDisabled(true);
  ...
  const experienceHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    stepSix('experience', e?.currentTarget?.value)
  }
  const gymHandler =  (e: React.ChangeEvent<HTMLInputElement>) => {
    stepSix('gym', e?.currentTarget?.value)
  }
  return (
    <div className="w-full md:w-1/2 p-6 mb-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 space-y-3">
      ...
      <AgeRangeInput max={30} min={0} value={props?.data?.stepSix?.experience} label="Experience" onChange={experienceHandler}/>
